...
public class Dataset : SafeHandleZeroOrMinusOneIsInvalid
{
	...
	public unsafe void AddData(float[] input, float[] expectedOutput)
	{
		fixed (float* inputPtr = input, expectedOutputPtr = expectedOutput)
		{
			Dataset_AddData(handle, inputPtr, input.Length, expectedOutputPtr, expectedOutput.Length);
		}
	}
	...
	[DllImport(DLL_NAME)]
	private static extern unsafe void Dataset_AddData(IntPtr ptr, float* input, int inputSize, float* output, int outputSize);
